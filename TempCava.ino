// Include the libraries we need
#include <SoftwareSerial.h>
#include <OneWire.h>
#include <DallasTemperature.h>
#include <LiquidCrystal.h>
#include <BigFont.h>
// Data wire is plugged into port 2 on the Arduino
#define ONE_WIRE_BUS 8
const int lcdD7Pin = 7; // LCD D7 pin
const int lcdD6Pin = 6; // LCD D6 pin
const int lcdD5Pin = 5; // LCD D5 pin
const int lcdD4Pin = 4; // LCD D4 pin
const int lcdEPin = 3; // LCD E Pin
const int lcdRSPin = 2; // LCD RS pin

LiquidCrystal lcd(lcdRSPin, lcdEPin, lcdD4Pin, lcdD5Pin, lcdD6Pin, lcdD7Pin); // construct LCD object
BigFont bigNum(&lcd); // construct BigNumbers object, passing to it the name of our LCD object

// Setup a oneWire instance to communicate with any OneWire devices (not just Maxim/Dallas temperature ICs)
OneWire oneWire(ONE_WIRE_BUS);

// Pass our oneWire reference to Dallas Temperature. 
DallasTemperature sensors(&oneWire);
//SoftwareSerial TempCava(7, 8);
int j=0;
/*
 * The setup function. We only start the sensors here
 */
void setup(void)
{
  lcd.begin(16,2); // setup LCD rows and columns
  bigNum.begin(); // set up BigNumbers
  lcd.clear(); // clear display
  // start serial port
  Serial.begin(9600);
  //TempCava.begin(9600);
  //Serial.println("Dallas Temperature IC Control Library Demo");

  // Start up the library
  sensors.begin();
}

/*
 * Main function, get and show the temperature
 */
void loop(void)
{ 
  // call sensors.requestTemperatures() to issue a global temperature 
  // request to all devices on the bus
  //Serial.print("Requesting temperatures...");
  sensors.requestTemperatures(); // Send the command to get temperatures
  //Serial.println("DONE");
  // After we got the temperatures, we can print them here.
  // We use the function ByIndex, and as an example get the temperature from the first sensor only.
  //Serial.print("Temperature for the device 1 (index 0) is: ");
  //Serial.print('$');
  float a=sensors.getTempCByIndex(0);
  Serial.println(a);
  lcd.clear();
  bigNum.displayLargeFloat(a);
  //Serial.println(";");
  //TempCava.print("$");
  //TempCava.print(j);
  //TempCava.print(',');
  //TempCava.println(int(sensors.getTempCByIndex(0)));  
  //j++;
  //TempCava.println(";");  
}
